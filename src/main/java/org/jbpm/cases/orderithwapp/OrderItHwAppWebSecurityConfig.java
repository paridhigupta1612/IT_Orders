...
public class OrderItHwAppWebSecurityConfig extends WebSecurityConfigurerAdapter {
    ...
    public void configureGlobal(AuthenticationManagerBuilder auth) throws Exception {
        ...
        auth.inMemoryAuthentication().withUser("adminUser").password("{noop}adminUser").roles("kie-server");
        auth.inMemoryAuthentication().withUser("executionUser").password("{noop}test1234!").roles("kie-server");
        
        //added by sanjeev
        auth.inMemoryAuthentication().withUser("admin").password("{noop}test1234!").roles("kie-server");
